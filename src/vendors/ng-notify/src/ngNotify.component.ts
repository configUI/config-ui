...
@Component({
    selector: 'notify',
    template: `<div [@shrinkOut]="fade" class="ngn {{'ngn-' + type}}"
    [ngClass]="{'ngn-top': position=='top','ngn-bottom':position=='bottom','ngn-fixed':location=='body',
    'ngn-absolute':location!='body','ngn-sticky':sticky==true}">
  <span class="ngn-message">{{notifyText}}</span>
    <span class="ngn-dismiss" (click)="dismiss()">×</span>
</div>`,
    styleUrls: ['ngNotify.css'],
    animations: [
        trigger('shrinkOut', [
            state('show', style({
                opacity: 1
